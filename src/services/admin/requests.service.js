...
export async function createRegistrationRequestService({
  name,
  email,
  phone,
  city,
...
export async function declineRegistrationRequestService(
  id,
  adminId,
  reviewNote,
) {
  ...
  sendRegistrationDeclinedEmail({
    ...
    console.error("Failed to send registration decline email:", err),
  );
...
export async function getVerificationRequestsService({
  status,
  skip = 0,
  take = 10,
} = {}) {
...
export async function createVerificationRequestService(
  ...
  if (existing)
    throw new Error("A pending verification request already exists.");
...
export async function declineVerificationRequestService(
  id,
  adminId,
  reviewNote,
) {
  ...
  sendVerificationDeclinedEmail({
    ...
    console.error("Failed to send verification decline email:", err),
  );
